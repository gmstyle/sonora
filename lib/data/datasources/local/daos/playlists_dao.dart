...
class PlaylistsDao extends DatabaseAccessor<AppDatabase> {
  ...
  Future<LocalPlaylist?> getPlaylist(int id) =>
      (select(db.localPlaylists)..where((t) => t.id.equals(id)))
          .getSingleOrNull();

  Future<int> createPlaylist(String name, {String? description}) =>
      into(db.localPlaylists).insert(LocalPlaylistsCompanion(
        name: Value(name),
  ...
  Future<void> updatePlaylist(int id, {String? name, String? description}) =>
      (update(db.localPlaylists)..where((t) => t.id.equals(id)))
  ...
  Future<void> deletePlaylist(int id) async {
    await (delete(db.playlistEntries)..where((t) => t.playlistId.equals(id)))
        .go();
  ...
  Future<void> addEntry(int playlistId, String videoId, int position) =>
      into(db.playlistEntries).insert(PlaylistEntriesCompanion(
  ...
  Future<void> removeEntry(int playlistId, String videoId) =>
      (delete(db.playlistEntries)
            ..where((t) =>
                t.playlistId.equals(playlistId) & t.videoId.equals(videoId)))
          .go();

  Future<void> reorderEntries(int playlistId, List<String> videoIds) async {
    for (var i = 0; i < videoIds.length; i++) {
      await (update(db.playlistEntries)
            ..where((t) =>
                t.playlistId.equals(playlistId) &
                t.videoId.equals(videoIds[i])))
