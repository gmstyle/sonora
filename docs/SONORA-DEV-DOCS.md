...
│   ├── datasources/
│   │   ├── local/
│   │   │   ├── database.dart           # Drift AppDatabase (schema v22) — pure Dart
...
## 5. Drift Database — Schema v25

### 5.1 Tables

| Table | PK | Key Fields | Notes |
|---|---|---|---|
| `liked_songs` | videoId | title, artist, thumbnailUrl, artistId, albumId, **artistsJson**, addedAt, isVideo, duration | v8: +artistId/albumId, v12: +isVideo, v14: +duration, **v21: +artistsJson** |
...
| `downloads` | videoId | title, artist, **artistsJson**, thumbnailUrl, localPath, format, fileSize, downloadedAt, status, isVideo, isExplicit, **collectionId**, **collectionType**, **collectionName**, **collectionIndex** | v5/v6: +title/artist/thumbnailUrl, v11: +isVideo, v15: +isExplicit, **v21: +artistsJson**, **v23: +collection\***, **v25: +collectionIndex** |
...
| `PlaybackRestoreController` | `playback_restore_controller.dart` | Cold-start restore from Drift + warm-resume stale-URL refresh; owns `RestoreStatus` / `savedPosition` / `awaitReady`. MediaSession / Android Auto `play()` calls `ensureReady()` (not only `awaitReady`) so a warm process refreshes a stale stream URL without opening the phone UI |
...
- **Dual-Path Playback Architecture:**
  - **Explicit User Downloads** (`StartDownloadUseCase`): Triggered by user action. Selects an audio stream via `StreamQualitySelector` using `downloadQuality` (never muxed/video). Files are saved permanently to disk (`/Sonora/` or `/Sonora/<collection>/`) and recorded in SQLite (`DownloadsTable`, including catalog `isVideo` metadata and optional `collectionId` / `collectionType` / `collectionName` / `collectionIndex` from bulk `batchId` — `collectionIndex` is the 0-based position in the source album/playlist/podcast list so completed groups keep source order). Re-downloads write to a `.part` staging file; the previous completed file and SQLite row are replaced only after the new file is fully written (`DownloadReplacement`). A failed replacement restores the previous completed row. `PlayVideoIdUseCase.resolveUrl` is the **single playback URL entry point** for all play paths (album, playlist, podcast, smart mix, radio, favorites, local playlists, Android Auto downloads browse, Play All download groups, play-next / add-to-queue, CLI `play`, skip/restore/recovery): order is completed library download → audio-only media-cache hit → live stream. Cast, library download acquisition, and `LocalAudioProxyServer` still resolve YouTube streams separately via `resolveStreamUrl` / `getStreamUrl`. Completed downloads are grouped in the UI via `downloadGroupsProvider` (persisted collection → inferred folder → Singles); default track order within a collection group uses `collectionIndex` ascending (legacy rows without an index fall back to `downloadedAt` desc).
